...
/* Two-arm hack for TRINA2... may be a better way to implement this via scope resolution in the future.  
 * To revert, uncomment next two lines and remove //TRINA HACK section.
 */
  //  std::string topic = contactManager->CreateFilter(model->GetScopedName(),
  //                      collisionNames);
  
  // BEGIN TRINA HACK
  std::string test_name = model->GetScopedName() + "/right_arm";
  std::string topic = contactManager->CreateFilter(test_name, collisionNames);
  if (topic == "") {
    test_name = model->GetScopedName() + "/left_arm";
    topic = contactManager->CreateFilter(test_name, collisionNames);
  }
  // END TRINA HACK
